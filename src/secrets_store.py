...
class SecretStore:
    ...
    def target(reference):
        if not isinstance(reference,str) or not re.fullmatch(r'[a-zA-Z0-9._/-]{1,160}',reference):
            raise ValueError(tr('Недопустимая ссылка на учётные данные'))
        return SecretStore.PREFIX+reference
    @staticmethod
    def api():
        if os.name!='nt':raise RuntimeError(tr('Диспетчер учётных данных Windows недоступен на этой платформе'))
    ...
    def put(self,reference,value):
        ...
        if len(raw)>2560:raise ValueError(tr('Учётные данные превышают допустимый в Windows размер'))
        ...
        try:
            if not api.CredWriteW(C.byref(entry),0):raise OSError(C.get_last_error(),tr('Не удалось записать в диспетчер учётных данных'))
        finally:
    ...
    def get(self,reference):
        ...
        if not api.CredReadW(target,1,0,C.byref(pointer)):
            raise OSError(C.get_last_error(),tr('Учётные данные по ссылке недоступны'))
    ...
    def delete(self,reference):
        ...
        if not api.CredDeleteW(target,1,0) and C.get_last_error()!=1168:
            raise OSError(C.get_last_error(),tr('Не удалось удалить из диспетчера учётных данных'))
