...
class OpenClawAdapter(AgentRuntimeAdapter):
    ...
    def detect(self):
        ...
        self.frontends = [
            {'id': 'openclaw-terminal', 'runtime_id': self.id, 'name': 'OpenClaw — локальный терминал',
             'type': 'terminal', 'optional': True, 'status': 'NOT INSTALLED'},
            {'id': 'openclaw-gateway', 'runtime_id': self.id, 'name': 'OpenClaw — шлюз (Gateway)',
             'type': 'gateway', 'optional': True, 'status': 'NOT INSTALLED', 'adapter_launch': True},
        ]
        if not self.command:
            return unsupported('OpenClaw не установлен (необязательный агент). Чтобы использовать его, установите OpenClaw и нажмите «Найти агенты заново».')
        try:
            ...
            return Result(Support.SUPPORTED if '--local' in self.tui_help else Support.DEGRADED,
                'OpenClaw найден' if '--local' in self.tui_help else
                'OpenClaw найден, но эта версия не поддерживает локальный терминал (tui --local). Обновите OpenClaw.', {'version': self.version})
        except Exception as exc:
            for frontend in self.frontends:
                frontend['status'] = 'UNSUPPORTED BY INSTALLED VERSION'
            return Result(Support.DEGRADED, 'OpenClaw найден, но не отвечает на проверку версии: ' + str(exc))
    ...
    def _configuration(self):
        ...
        if included(doc):
            raise ValueError('Настройки OpenClaw (' + self.get_config_locations().data['user'] + ') подключают другие файлы через $include — '
                'Station не может изменять их автоматически. Укажите для OpenClaw отдельную папку настроек Station.')
        if not isinstance(doc, dict):
            raise ValueError('Файл настроек OpenClaw повреждён или имеет неожиданный формат: ' + self.get_config_locations().data['user'] +
                '. Исправьте его или укажите другую папку настроек.')
        return doc

    def configure_model_provider(self, models):
        if not self.get_capabilities().data['provider_sync']:
            return unsupported('OpenClaw не найден или его версия не поддерживает проверку настроек (config validate). Установите или обновите OpenClaw и нажмите «Найти агенты заново».')
        self._configuration()
    ...
    def configure_model_binding(self, model):
        if not self.get_capabilities().data['provider_sync']:
            return unsupported('Настройка модели для OpenClaw недоступна. Установите или обновите OpenClaw и нажмите «Найти агенты заново».')
    ...
    def validate_configuration(self):
        if not self.command or 'validate' not in self.config_help:
            return unsupported('Проверка настроек OpenClaw недоступна: OpenClaw не установлен или его версия не поддерживает config validate. Обновите OpenClaw.')
        try:
            probe(self.command + ['config', 'validate'], timeout=40, env=self.process_environment())
            return Result(Support.SUPPORTED, 'Настройки OpenClaw в порядке (проверено самим OpenClaw)')
        except Exception as exc:
    ...
    def start(self, workspace=None, model=None):
        if not self.command or '--local' not in self.tui_help:
            return unsupported('OpenClaw не установлен или эта версия не поддерживает локальный терминал (tui --local). Установите или обновите OpenClaw.')
        if model:
            ...
            if not isinstance(binding, dict) or binding.get('primary') != provider_id(model) + '/' + model.backend_model_id:
                return unsupported('В настройках OpenClaw выбрана другая модель. Нажмите «Синхронизировать с агентами», просмотрите изменения и запустите снова.')
        checked = self.validate_configuration()
        if not checked.ok:
            return checked
        try:
            return Result(Support.SUPPORTED, 'Локальный терминал OpenClaw открыт', supervisor.start('agent:' + self.id,
    ...
    def launch_frontend(self, frontend, workspace=None, model=None):
        if frontend.get('type') != 'gateway' or not self.command or '--bind' not in self.gateway_help:
            return unsupported('Шлюз OpenClaw недоступен: OpenClaw не установлен или эта версия не поддерживает gateway run --bind. Обновите OpenClaw.')
        checked = self.validate_configuration()
        if not checked.ok:
            return checked
        doc = self._configuration()
        if doc.get('gateway', {}).get('mode') != 'local':
            return unsupported('Шлюз OpenClaw не настроен на локальный режим. Выполните настройку OpenClaw (gateway.mode=local) и запустите шлюз снова.')
        args = self.command + ['gateway', 'run', '--bind', 'loopback', '--tailscale', 'off']
        port = self.settings.get('gateway_port')
        if port is not None:
            if type(port) is not int or not 1024 <= port <= 65535:
                return Result(Support.ERROR, f'Порт шлюза OpenClaw указан неверно ({port}). Укажите целое число от 1024 до 65535 в настройках агента.')
            args += ['--port', str(port)]
        try:
            return Result(Support.SUPPORTED, 'Шлюз OpenClaw запущен в фоне', supervisor.start('frontend:' + frontend['id'],
    ...
    def smoke(self, model, workspace, timeout=180, configuration_path=None):
        ...
        if not self.command or not all(flag in self.exec_help for flag in flags):
            return unsupported('Проверка OpenClaw через модель недоступна: OpenClaw не установлен или его версия не поддерживает agent exec. Обновите OpenClaw.')
        try:
            ...
            with tempfile.TemporaryDirectory(prefix='openclaw-smoke-', dir=workspace) as temporary:
                ...
                if configuration_path:
                    ...
                    if not isinstance(binding, dict) or binding.get('primary') != reference or not provider:
                        return Result(Support.ERROR, 'Сохранённые настройки OpenClaw не указывают на выбранную модель Station. Нажмите «Синхронизировать с агентами».')
                else:
                ...
                return Result(Support.SUPPORTED if passed else Support.ERROR,
                    'Проверка OpenClaw через модель пройдена' if passed else
                    f'Проверка OpenClaw через модель не пройдена (код выхода {p.returncode}). Убедитесь, что модель запущена, и посмотрите вывод проверки.',
