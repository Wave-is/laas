...
class HermesAdapter(AgentRuntimeAdapter):
    ...
    def detect(self):
        ...
        self.frontends = [{'id': 'hermes-cli', 'runtime_id': self.id, 'name': 'Hermes — терминал',
        ...
        if executable:
            try:
                ...
                error = 'Hermes Agent найден, но не отвечает на проверку версии: ' + str(exc)
        ...
        self.frontends.extend([
            ...
            {'id': 'hermes-gateway', 'runtime_id': self.id, 'name': 'Hermes — шлюз (Gateway / Telegram)', 'type': 'gateway',
             'status': 'INSTALLED' if 'gateway' in self.help_text else 'NOT INSTALLED'},
            {'id': 'hermes-dashboard', 'runtime_id': self.id, 'name': 'Hermes — панель (Dashboard)', 'type': 'dashboard',
             'status': 'INSTALLED' if 'dashboard' in self.help_text else 'NOT INSTALLED'},
        ])
        return Result(Support.DEGRADED if error else Support.SUPPORTED if executable else Support.UNSUPPORTED, message=error or ('Hermes Agent найден' if executable else 'Hermes Agent не установлен. Установите его и нажмите «Найти агенты заново».'), data={
    ...
    def configure_model_provider(self, models):
        if not self.command:
            return unsupported('Hermes Agent не установлен. Установите его и нажмите «Найти агенты заново».')
        ...
        try:
            ...
            for other in locations['alternatives']:
                if read_document(Path(other), {}).get('model') != current.get('model'):
                    return Result(Support.DEGRADED, 'Настройки Hermes в разных папках различаются: ' +
                        locations['user'] + ' и ' + other + '. Оставьте одну папку настроек или укажите одинаковую модель.')
            return Result(Support.SUPPORTED, 'Настройки Hermes в порядке')
        except Exception as exc:
    ...
    def start(self, workspace=None, model=None):
        ...
        if model:
            if '--model' not in self.help_text:
                return unsupported('Эта версия Hermes Agent не поддерживает выбор модели при запуске (--model). Обновите Hermes Agent.')
            args += ['--model', model.backend_model_id]
            if '--provider' not in self.help_text:
                return unsupported('Эта версия Hermes Agent не поддерживает выбор провайдера при запуске (--provider). Обновите Hermes Agent.')
    ...
    def smoke(self, model, workspace, timeout=180, configuration_path=None):
        ...
        if configuration_path:
            ...
            if not provider:
                return Result(Support.ERROR, f'В сохранённых настройках Hermes ({configuration_path}) нет подключения к модели Station. Нажмите «Синхронизировать с агентами».')
        ...
        try:
            ...
            return Result(Support.SUPPORTED if passed else Support.ERROR, 'Проверка Hermes через модель пройдена' if passed else
                f'Проверка Hermes через модель не пройдена (код выхода {p.returncode}). Убедитесь, что модель запущена, и посмотрите вывод проверки.',
