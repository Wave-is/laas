...
def result_message(value):
    ...
    return {'Model answered a real health request': 'Модель готова: проверочный запрос выполнен',
            'Loaded model answered a health request': 'Модель уже загружена и отвечает',
            'Model unloaded': 'Модель выгружена', 'Frontend process started': 'Интерфейс агента запущен',
            'Terminal opened': 'Терминал агента открыт', 'Owned process stopped': 'Процесс остановлен',
            'No owned process is running': 'Управляемый процесс уже остановлен',
            'No driver changes required': 'Изменение режимов GPU не требуется',
...
class ControlCenter(AgentControls, StartupControls, ServiceControls, GpuControls, TrayControls, ctk.CTk):
    def __init__(self, start_minimized=False, no_tray=False, skip_startup=False):
        ...
        self.title(APP_NAME + ' Control Center')
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkButton(sidebar, text='Быстрое меню', fg_color=EDGE, command=self._show_quick_menu).pack(fill='x', padx=20, pady=(25, 0))
    ...
    def _build_station(self):
        ...
        for column, (key, label) in enumerate((('backend', 'СЕРВЕР МОДЕЛЕЙ'), ('model', 'ЗАГРУЖЕННАЯ МОДЕЛЬ'), ('agent', 'ИНТЕРФЕЙС АГЕНТА'))):
        ...
        card = self.card(page, 'Запуск и остановка')
        row = self.row(card)
        ...
        self.button(row, 'Запустить', lambda: self._run_selection(self.model_combo, gpu_mode_manager.apply_model_profile_only), True, width=110)
        self.button(row, 'Выгрузить', lambda: self.worker(lambda: gpu_mode_manager.apply_model_profile_only('none')), width=110)
        row = self.row(card)
        ...
        self.button(row, 'Открыть', self._launch_frontend, True, width=110)
        self.button(row, 'Остановить', lambda: self._run_selection(self.frontend_combo, self.controller.stop_frontend), width=110)
        self.combination_label = ctk.CTkLabel(card, text='Выберите модель и нажмите «Запустить».', anchor='w', justify='left', text_color=MUTED)
        ...
        card = self.card(page, 'Сохранённые комбинации')
        row = self.row(card)
        ...
        row = self.row(card)
        self.button(row, 'Сохранить комбинацию', self._save_preset)
        self.button(row, 'Как проверить', self._testing_guide)
    ...
    def _build_hardware(self):
        ...
        self.button(row, 'Применить профиль', self._preview_gpu, True)
    ...
    def _build_models(self):
        ...
        row = self.row(card)
        self.button(row, 'Редактировать профили', lambda: self.edit_document('model_profiles.yaml'), True)
        self.button(row, 'Синхронизировать с агентами', self._sync_models)
        row = self.row(card)
        ...
        self.button(row, 'Проверить модель', self._qualify_model)
    ...
    def _build_agents(self):
        ...
        row = self.row(card)
        self.button(row, 'Повторить обнаружение', lambda: self.worker(self.controller.discover_agents, self._agents_discovered), True)
        self.button(row, 'Runtime', lambda: self.edit_document('agent_runtimes.yaml'))
        self.button(row, 'Профили интерфейсов', lambda: self.edit_document('agent_frontends.yaml'))
    ...
    def _build_services(self):
        ...
        card = self.card(page, 'Сервер локальных моделей', 'Запускается вместе с моделью и обслуживает выбранные агенты. Здесь можно отдельно управлять сервером llama-swap.')
        row = self.row(card)
        self.button(row, 'Запустить сервер', lambda: self.worker(lambda: {'Success': pm.start_llama_swap(), 'Message': 'Запуск сервера завершён'}), True)
        self.button(row, 'Остановить сервер', lambda: self.worker(lambda: {'Success': pm.stop_llama_swap(), 'Message': 'Остановка сервера завершена'}))

    def _build_settings(self):
        page = self.page('Настройки')
        self._build_startup_settings(page)
        self._build_tray_settings(page)
        card = self.card(page, 'Пути и рабочая папка', str(data_dir()))
        self.path_entries = {}
        for key, title, directory in [('workspace', 'Рабочая папка', True),
            ('llama_swap_executable', 'llama-swap.exe', False), ('llama_server_executable', 'llama-server.exe', False),
            ('llama_swap_config', 'Конфигурация llama-swap', False)]:
            ctk.CTkLabel(card, text=title, anchor='w', text_color=MUTED).pack(fill='x', padx=20)
            row = self.row(card)
            entry = ctk.CTkEntry(row, width=540)
            entry.insert(0, config.get(key, ''))
            entry.pack(side='left', fill='x', expand=True, pady=(0, 8), padx=(0, 12))
            self.path_entries[key] = entry
            ctk.CTkButton(row, text='Обзор', width=90, fg_color=EDGE,
                command=lambda e=entry, d=directory: self._browse(e, d)).pack(side='left')
        row = self.row(card)
        self.button(row, 'Сохранить пути', self._save_paths, True)
        ...
        row = self.row(card)
        self.button(row, 'Установить GPU helper', self._install_helper)
        self.button(row, 'Вернуть подтверждение GPU', self._reset_gpu_confirmation)
        row = self.row(card)
        self.button(row, 'GPU-профили', lambda: self.edit_document('hardware_profiles.yaml'))
        self.button(row, 'Пресеты', lambda: self.edit_document('station_presets.yaml'))
    ...
    def _save_paths(self):
        try:
            config.update({key: entry.get().strip() for key, entry in self.path_entries.items()})
            self.status_label.configure(text='Пути сохранены', text_color=ACCENT)
    ...
    def _refresh_models_text(self):
        ...
        for model in profile_storage.model_profiles.values():
            if model.id == 'none':
                continue
            exists = Path(model.weights_path).is_file()
            lines.append(f'{model.name}\n  {model.id} · {model.status}\n  Файл: {"найден" if exists else "не найден"} | Контекст: {model.context:,}\n  Smoke: {"проверен" if model.qualified else "не выполнен"} | Backend: {model.backend}\n')
    ...
    def review(self, title, content, apply=None, callback=None):
        ...
        if apply:
            self.button(row, 'Применить показанные изменения', lambda: (window.destroy(), self.worker(apply, callback)), True)
    ...
    def _sync_models(self):
        ...
        for id, adapter in self.controller.adapters.items():
            try:
                ...
                if not model or model.id == 'none':
                    raise ValueError('Сначала запустите модель: синхронизация включает настоящий smoke-запрос.')
        ...
        self.review('Синхронизация моделей с агентами', content, apply)
    ...
    def _qualify_model(self):
        ...
        def run():
            ...
            return {'Success': report['passed'], 'Message': 'Проверка завершена: ' + str(target), 'Report': report}
        def done(result):
            self._refresh_models_text()
            self.status_label.configure(text=result['Message'][:150], text_color=ACCENT if result.get('Success') else '#f8ad88')
            self.review('Результаты проверки модели', json.dumps(result, ensure_ascii=False, indent=2))
        self.worker(run, done)
    ...
    def _launch_frontend(self, frontend_id=None):
        ...
        if model and model.id != 'none' and adapter and adapter.manifest.get('provider_sync', True):
            ...
            if preview.status != 'IN SYNC':
                ...
                self.review('Подключить выбранную модель и открыть агент', preview.diff, apply, self._agent_frontend_done)
                return
        self.worker(lambda: self.controller.launch_frontend(frontend_id), self._agent_frontend_done)
    ...
    def _run_selection(self, combo, action):
        selected = combo.get()
        self.worker(lambda: action(selected))

    def worker(self, action, callback=None):
        if self.busy:
            self.status_label.configure(text='Дождитесь завершения текущего действия.', text_color=MUTED)
            return
        self.busy = True
        self._refresh_agent_launch_states()
        ...
        self.status_label.configure(text='Выполняется…', text_color=MUTED)
        def work():
            try:
                self.events.put(('result', action(), callback))
            except Exception as exc:
                self.events.put(('result', {'Success': False, 'Message': str(exc)}, None))
    ...
    def _poll(self):
        while not self.stop_event.is_set():
            try:
                ...
                if backend:
                    try:
                        running = [r['model'] for r in gpu_mode_manager.engine.request('/running').get('running', []) if r.get('state') == 'ready']
                    except Exception:
                        pass
                if not self.events.full():
                    ...
                    self.events.put(('telemetry', (top, backend, running, service_states, frontend_running), None))
            except Exception as exc:
                if not self.events.full():
        ...
        try:
            while True:
                ...
                if kind == 'result':
                    ...
                    if callback:
                        self.status_label.configure(text='Готово', text_color=ACCENT)
                        callback(value)
                    else:
                        self.status_label.configure(text=result_message(value)[:150],
    ...
    def _update_telemetry(self, top, backend, running, service_states, frontend_running=None):
        self.topology = top
        ...
        self.topology_label.configure(text=f'GPU: {top.gpu_count}  ·  CUDA P2P: {p2p}  ·  NVLink: {nvlink}\nТрафик NVLink не измерялся.' +
            ('\n' + top.discovery_error if top.discovery_error else '') + ('\nCPU-only: выделенные GPU не обнаружены.' if not top.devices else ''))
        self.combination_label.configure(text='Модель готова к работе.' if running else
            'Сервер доступен. Выберите модель для загрузки.' if backend else 'Выберите модель и нажмите «Запустить».')
    ...
    def _update_dashboard(self, top, backend, running):
        ...
        value.configure(text='Доступен' if backend else 'Остановлен', text_color=ACCENT if backend else MUTED)
        detail.configure(text='Есть ответ сервера' if backend else 'Запустится вместе с моделью')
        ...
        detail.configure(text=f'Готово моделей: {len(running)}' if running else 'Выберите модель ниже')
        ...
        detail.configure(text='Запущен Station' if self.frontend_running.get(fid) else
    ...
    def _dispatch_tray(self, action, value=None):
        if action == 'page':
            ...
            self.worker(lambda: {'Success': pm.start_llama_swap(), 'Message': 'Запуск backend завершён'})
        elif action == 'backend_stop':
            self.worker(lambda: {'Success': pm.stop_llama_swap(), 'Message': 'Остановка backend завершена'})
