...
class StartupControls:
    def _build_startup_settings(self, page):
        ...
        card = self.card(page, 'Запуск Station и компонентов',
            'Windows запускает Station при входе в вашу учётную запись. Выбранные ниже компоненты запускаются при каждом запуске Station.')
        self.windows_startup = WindowsStartup()
        self.windows_startup_var = tk.BooleanVar(value=False)
        self.windows_startup_check = ctk.CTkCheckBox(card, text='Запускать Station при входе в Windows',
        ...
        self.startup_vars = {}
        for key, label in [('minimized', 'Открывать Station свёрнутой в трей'),
                           ('enabled', 'Автоматически запускать выбранные ниже компоненты'),
                           ('stop_on_error', 'Останавливать дальнейший запуск при ошибке')]:
        ...
        ctk.CTkLabel(card, text='Выберите компоненты ниже и нажмите «Сохранить запуск».',
            text_color=MUTED, anchor='w').pack(fill='x', padx=20, pady=(4, 0))
        row = self.row(card)
        ctk.CTkLabel(row, text='Задержка, секунд (0–300)').pack(side='left', padx=(0, 15))
        ...
        ctk.CTkLabel(card, text='Локальные службы', anchor='w', text_color=MUTED).pack(fill='x', padx=20, pady=(12, 0))
        ...
        self.startup_cancel_button = ctk.CTkButton(self.startup_banner, text='Отменить запуск', width=145,
    ...
    def _windows_startup_loaded(self, state):
        ...
        self.windows_startup_hint.configure(text='Автозагрузка Windows включена для текущего пользователя.' if state['enabled'] else
            'Автозагрузка Windows выключена. Галочка сохраняется сразу; права администратора не нужны.')
    ...
    def _refresh_startup_choices(self, initial=None):
        ...
        for key, area, attr, rows in groups:
            ...
            for id in dict.fromkeys([*order, *sorted(selected)]):
                ...
                label = row.get('name', id) + ('' if available else ' — недоступен')
                ...
                variables[id] = var
            if not variables:
                ctk.CTkLabel(area, text='Нет локальных служб. Добавьте их в разделе «Службы».' if key == 'service_ids' else
                    'Идёт обнаружение агентов…', text_color=MUTED).pack(anchor='w')
    ...
    def _save_startup_settings(self):
        try:
            ...
            if settings['enabled'] and any(self.controller.frontends.get(id, {}).get('status') != 'INSTALLED' for id in settings['frontend_ids']):
                raise ValueError('Уберите недоступные интерфейсы из автозапуска или установите их.')
            ...
            if settings['enabled'] and any(id not in services or services[id].get('type', 'local') != 'local' for id in settings['service_ids']):
                raise ValueError('Уберите отсутствующие или удалённые службы из автозапуска.')
    ...
    def _schedule_startup(self):
        ...
        def tick(remaining):
            if self.stop_event.is_set() or self.startup_cancel.is_set():
                return
            self.startup_banner_label.configure(text=f'Автозапуск через {remaining} с. Компонентов: {len(startup_steps(settings))}.')
            if remaining:
    ...
    def _cancel_startup(self):
        ...
        self.startup_banner_label.configure(text='Автозапуск отменён. Текущее действие завершится; следующие не начнутся.')
        self.startup_cancel_button.configure(state='disabled')
        if self.startup_pending:
            self.startup_pending = False
            self._startup_done({'Success': False, 'Message': 'Автозапуск отменён до запуска компонентов.', 'Steps': []})
    ...
    def _show_startup_result(self):
        ...
        lines = [report.get('Message', 'Автоматический запуск компонентов ещё не выполнялся.')]
        lines += [('✓ ' if row.get('Success') else 'Ошибка: ') + row['name'] + '\n' + row.get('Message', '') for row in report.get('Steps', [])]
        self.review('Результат автоматического запуска', '\n\n'.join(lines))
